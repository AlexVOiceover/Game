...
// Function to check if all letters have been guessed. When spots a letter from arrSecretword that its not on guessedLetters returns false
function checkWin() {
  for (let i = 0; i < arrSecretword.length; i++) {
    if (!guessedLetters.includes(arrSecretword[i])) {
      return false;
    }
  }
  return true;
